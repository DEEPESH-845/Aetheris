import { useSimulationStore, type MitreTactic, type TelemetryEvent } from '@/store/useSimulationStore';

type Store = ReturnType<typeof useSimulationStore.getState>;
...
const TICK_MS = 800; // ~1.25 ticks/sec, ~3-4 events/sec across generators

/** One step of the local simulation. Pure function of the store plus a tick counter, so it can run under fake timers. */
export function tick(st: Store, tick: number) {
...
/** Seeds one incident on the first visit of a browser session so the loop is visible without finding Sandbox Lab. */
export function seedFirstIncident() {
  const st = useSimulationStore.getState();
  if (st.incidentLog.length > 0) return;
  try {
    if (sessionStorage.getItem(SEED_KEY) === '1') return;
...
export function startGenerator() {
  if (interval) return;
  interval = setInterval(() => tick(useSimulationStore.getState(), ++count), TICK_MS);
}
...
export const isGeneratorRunning = () => interval !== null;
